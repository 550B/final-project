...
void GameLayer::initToolLayer()
{
	...
	//金币数字
	money = 10;
	auto moneyText = patch::to_string(money);//转换为string
	...
	playHpBar->setType(ProgressTimer::Type::BAR);//cc.PROGRESS_TIMER_TYPE_RADIAL为圆形	cc.PROGRESS_TIMER_TYPE_BAR为条形
	playHpBar->setMidpoint(Point(0, 0.4f));//水平方向的起点为最左边，则进度条减少的方向为从右到左
	playHpBar->setBarChangeRate(Point(1, 0));//表示水平方向有进度，垂直方向无进度
	playHpBar->setPercentage(100);//设置进度0-100
...
*/
	...
	// back
	//创建背景图精灵的子节点
	Button* back = Button::create("Pictures/pause.png", "Pictures/pause.png", "");//表示一个正常状态，一个选择状态，一个不可点击状态
	back->setScale(2);
