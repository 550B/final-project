...
bool HardMap1::init()
{
    ...
    droad = map->getObjectGroup("droad");//无人机 无阻隔
   ...
   ValueVector ravalues = aroad->getObjects();
    for (Value value : ravalues)
    {
        ...
        aroad_path.push_back(Vec4(valueMap["x"].asFloat(), valueMap["y"].asFloat(), valueMap["width"].asFloat(), valueMap["height"].asFloat()));//将路径点保存到路径中
    }
    ValueVector rbvalues = broad->getObjects();
    for (Value value : rbvalues)
    {
        ValueMap valueMap = value.asValueMap();//获得属性值：Value转换成ValueMap       
        broad_path.push_back(Vec4(valueMap["x"].asFloat(), valueMap["y"].asFloat(), valueMap["width"].asFloat(), valueMap["height"].asFloat()));//将路径点保存到路径中
    }
    ValueVector rcvalues = croad->getObjects();
    for (Value value : rcvalues)
    {
        ValueMap valueMap = value.asValueMap();//获得属性值：Value转换成ValueMap       
        croad_path.push_back(Vec4(valueMap["x"].asFloat(), valueMap["y"].asFloat(), valueMap["width"].asFloat(), valueMap["height"].asFloat()));//将路径点保存到路径中
    }
    ValueVector rdvalues = droad->getObjects();
    for (Value value : rdvalues)
    {
        ValueMap valueMap = value.asValueMap();//获得属性值：Value转换成ValueMap       
        droad_path.push_back(Vec4(valueMap["x"].asFloat(), valueMap["y"].asFloat(), valueMap["width"].asFloat(), valueMap["height"].asFloat()));//将路径点保存到路径中
    }
    ...
    enemy_ground->setPosition(Vec2(aroad_path[0].x + aroad_path[0].z / 2, aroad_path[0].y + aroad_path[0].w / 2));
    ...
    for (int i = 1; i < (int)aroad_path.size(); i++)//遍历其他路径点
    {
        ...
        float lenth = sqrtf((aroad_path[i - 1].x - aroad_path[i].x) * (aroad_path[i - 1].x - aroad_path[i].x) + (aroad_path[i - 1].y - aroad_path[i].y) * (aroad_path[i - 1].y - aroad_path[i].y));//后一个减去前一个地址，再取得长度
        MoveTo* moveTo = MoveTo::create(lenth / 100, Vec2(aroad_path[i].x + aroad_path[i].z / 2, aroad_path[i].y + aroad_path[i].w / 2));//匀速
