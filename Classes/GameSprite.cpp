...
bool GameSprite::init()
{
    ...
    touchListener->onTouchBegan = CC_CALLBACK_2(GameSprite::onTouchBegan, this);
    touchListener->onTouchEnded = CC_CALLBACK_2(GameSprite::onTouchEnded, this);
...
bool GameSprite::onTouchBegan(Touch* touch, Event* event)
{
...
void GameSprite::onTouchEnded(Touch* touch, Event* event)
{
    auto target = static_cast<Sprite*>(event->getCurrentTarget());
    if (target == sprite1)
    {
