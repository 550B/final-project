...
Scene* NormalMap1::createScene()
{
    ...
    auto spr = GanYuanShooter::create();
    scene->addChild(spr);
    layer->ganyuanShooter = spr;
    layer->initGanyuanRoadSp();
    return scene;
...
bool NormalMap1::init()
{
    ...
    //对象层
    TMXObjectGroup* road = map->getObjectGroup("road");
    TMXObjectGroup* towers = map->getObjectGroup("towers");
    TMXObjectGroup* grounds = map->getObjectGroup("grounds");
    
    this->addChild(map, 0);
    this->mapLayer = layer_normalmap;
    
    //定义一个vector存储路径
    std::vector<Vec2> road_path;
    std::vector<Vec2> tmp_size;

    //获取所有对象
    ValueVector values = road->getObjects();
...
void NormalMap1::initGanyuanRoadSp() {

    TMXObjectGroup* towers =this->map->getObjectGroup("towers");
    int width = 100;
    int height = 100;
    std::vector<Rect> towerRect;
    ValueVector tower_values = towers->getObjects();
    for (Value value : tower_values)
    {
        ValueMap valueMap = value.asValueMap();//获得属性值：Value转换成ValueMap 
        Vec2 v = Vec2(valueMap["x"].asFloat(), valueMap["y"].asFloat());
        towerRect.push_back(Rect(v.x - width / 2, v.y - height / 2, width, height));//将路径点保存到路径中
    }
    this->ganyuanShooter->setTowerRect(towerRect);
